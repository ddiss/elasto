...
struct abb_fwrite_multi_data_ctx {
	struct abb_fwrite_multi_state *parent_state;
	uint64_t this_off;
	uint64_t this_len;
	struct op *op;
	struct event *ev_tx;
				...
				void *priv)
{
	...
	if ((need > ABB_MAX_PART)
			 ...
			 || (data_ctx->parent_state->src_data == NULL)
			 || ((data_ctx->this_off + stream_off + need)
				> data_ctx->parent_state->src_data->len)) {
		dbg(0, "failed write len sanity check!\n");
		ret = -EINVAL;
		goto err_out;
	}

	src_data = data_ctx->parent_state->src_data;
...
static int
abb_fwrite_multi_data_setup(struct abb_fwrite_multi_state *multi_state,
			    uint64_t this_off,
			    uint64_t this_len,
			    struct abb_fwrite_multi_data_ctx **_data_ctx)
{
	...
	if (multi_state->src_data->type == ELASTO_DATA_IOV) {
		ret = elasto_data_cb_new(this_len,
					 abb_fwrite_multi_iov_data_out_cb,
					 0, NULL, data_ctx,
					 &data_ctx->this_data);
	} else if (multi_state->src_data->type == ELASTO_DATA_CB) {
		ret = elasto_data_cb_new(this_len,
...
static void
abb_fwrite_multi_error_set(struct abb_fwrite_multi_state *multi_state,
		      int ret)
{
			     ...
			     void *priv)
{
	...
	struct abb_fwrite_multi_state *multi_state = finish_state->multi_state;

	ret = elasto_conn_op_rx(finish_state->ev_tx);
	if (ret < 0) {
		dbg(2, "block list put failed: %s\n", strerror(-ret));
		abb_fwrite_multi_error_set(multi_state, ret);
...
static int
abb_fwrite_multi_finish(struct abb_fwrite_multi_state *multi_state)
{
	int ret;
...
static void
abb_fwrite_multi_tx_pipe_fill(struct abb_fwrite_multi_state *multi_state);
			 ...
			 void *priv)
{
	int ret;
	struct abb_fwrite_multi_data_ctx *data_ctx = priv;
	struct abb_fwrite_multi_state *multi_state = data_ctx->parent_state;
	...
	abb_fwrite_multi_tx_pipe_fill(data_ctx->parent_state);
...
static int
abb_fwrite_multi_tx(struct abb_fwrite_multi_state *multi_state,
		    uint64_t this_off,
		    uint64_t this_len)
{
	int ret;
		 ...
		 uint64_t max_io)
{
	...
	abb_fwrite_multi_tx_pipe_fill(multi_state);
